...
const {id, read_message_id} = defineProps({
  id: {
    type: Number
  },
  read_message_id: {
...
onMounted(async () => {
  await messageAdapter.getHistory(id, read_message_id)
  title.value = getTitle()
})

const sendMessage = async () => {
  const message = messageModel.value
  const resp = await messageAdapter.sendMessage(id, message)
  messageModel.value = ''
  await messageAdapter.getHistory(id, read_message_id)
}
...
<template>
  <div v-if="messagesStore.messages[id]" class="flex-1 p:2 sm:p-6 justify-between flex flex-col h-screen">
    ...
    <div id="messages"
      ...
      <div class="chat-message" v-for="messageKey in Object.keys(messagesStore.messages[id]??{})">
        <div class="flex items-end"
             :class="{'justify-end': messagesStore.messages[id][messageKey].sender_user === authStore.getUserId}">
          <div class="flex flex-col space-y-2 text-xs max-w-xs mx-2 items-start"
               :class="{'order-2': messagesStore.messages[id][messageKey].sender_user !== authStore.getUserId}">
            <div><span class="px-4 py-2 rounded-lg inline-block"
                       :class="{'bg-blue-600 text-white': messagesStore.messages[id][messageKey].sender_user === authStore.getUserId,
                       'bg-gray-300 text-gray-600': messagesStore.messages[id][messageKey].sender_user !== authStore.getUserId}">
              {{ messagesStore.messages[id][messageKey].body }}
            </span></div>
          </div>
          <img
              src="https://images.unsplash.com/photo-1549078642-b2ba4bda0cdb?ixlib=rb-1.2.1&amp;ixid=eyJhcHBfaWQiOjEyMDd9&amp;auto=format&amp;fit=facearea&amp;facepad=3&amp;w=144&amp;h=144"
